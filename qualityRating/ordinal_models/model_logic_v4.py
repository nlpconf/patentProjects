...
class Model_v4(BaseModel):
    ...
    def train_step(self,batch,e):
        ...
        #batch_similar_docs = np.array(batch['similar_docs_' + 'content'])
        ...
        feed_dict.update({ph: data for ph, data in zip(self.model.doc_actual_sent_lengths, batch_input_actual_sent_lengths)})
        ...
        feed_dict.update({ph: data for ph, data in zip(self.model.similar_doc_actual_sent_lengths, batch_similar_actual_sent_lengths)})
    ...
    def saveModel(self, export_path):
        ...
        if self.builder == None:
            ...
            for i,name in enumerate(self.config.text_field_names):
                ...
                input_tensors[name + '_actual_sent_lengths'] = tf.saved_model.utils.build_tensor_info(
                self.model.doc_actual_num_sents[i])
                input_tensors[name + '_similar_docs'] = tf.saved_model.utils.build_tensor_info(self.model.similar_docs[i])
                input_tensors[name + '_similar_doc_actual_num_sents'] = tf.saved_model.utils.build_tensor_info(
                    self.model.similar_doc_actual_num_sents[i])
                input_tensors[name + '_similar_doc_actual_sent_lengths'] = tf.saved_model.utils.build_tensor_info(
                    self.model.similar_doc_actual_sent_lengths[i])
            #doc_cls_predictions_tensor_info = tf.saved_model.utils.build_tensor_info(self.model.doc_cls_predictions)
        ...
        with graph.as_default():
            ...
            with sess.as_default():
                ...
                for name in config.text_field_names:
                    ...
                    input_tensors[name + '_actual_sent_lengths'] = graph.get_tensor_by_name(name + '_doc_actual_sent_lengths' + ":0")

                    input_tensors[name + '_similar_docs'] = graph.get_tensor_by_name(name + '_similar_docs' + ":0")
                    input_tensors[name + '_similar_doc_actual_num_sents'] = graph.get_tensor_by_name(
                        name + '_similar_doc_actual_num_sents' + ":0")
                    input_tensors[name + '_similar_doc_actual_sent_lengths'] = graph.get_tensor_by_name(
                        name + '_similar_doc_actual_sent_lengths' + ":0")
                ...
                for batch in data_iterator(test_data, all_data, config.text_field_names, ids = test_ids, batch_size=config.batch_size,
                    ...
                    for name in config.text_field_names:
                        ...
                        input_data[name + '_actual_sent_lengths'] = batch[name + '_actual_sent_lengths']
                        input_data[name + '_similar_docs'] = batch[name + '_similar_docs']
                        input_data[name + '_similar_doc_actual_num_sents'] = batch[name + '_similar_docs_actual_num_sents']
                        input_data[name + '_similar_doc_actual_sent_lengths'] = batch[name + '_similar_docs_actual_sent_lengths']
